...
int main(int argc, char** argv) {
   ...
   if (world_rank == 0) {
    ...
    MPI_Scatter((world_rank == 0 ? A_mpi : nullptr), chunk * N, MPI_DOUBLE, local_A, chunk * N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    s0 = MPI_Wtime();
    parallel_PDE_async(local_A, local_C, chunk, N, T, D, k, lambda, ux, uy, dt, dx, dy,
                        world_rank, world_size);
    s1 = MPI_Wtime();
    if (world_rank == 0) printf("Parallel PDE (async) time: %.6f seconds\n", s1 - s0);
    MPI_Gather(local_C, chunk * N, MPI_DOUBLE, CnewPar, chunk * N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    if (world_rank == 0) {
        s0 = MPI_Wtime();
        check_matrix_equal(Cseq, CnewPar, N * N);
        s1 = MPI_Wtime();
        printf("Check async: seq vs parallel valid in %.6f seconds\n", s1 - s0);
    }
    if (world_rank == 0) {
        free(A_mpi);
        free(Cseq);
